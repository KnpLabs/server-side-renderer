...
| `WORKER_RENDERER_CHROME_PAGE_LOAD_WAIT_UNTIL`  |           | Defines Puppeteer's page load wait condition. Default `networkidle0` keeps the historical behavior. `networkidle2` or `load` can reduce render time for pages with long-lived requests, but may return before late async content is rendered. |

`--disable-dev-shm-usage` keeps the historical Chrome default. It can move Chrome shared-memory files to disk and increase I/O wait on hosts where `/dev/shm` is available and correctly sized; remove it from `WORKER_RENDERER_CHROME_OPTIONS` only after validating the runtime container/host shared-memory setup.
